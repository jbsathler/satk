...
DefTableKey k, int arg_num, Environment scope, CoordPtr pos)
{
  ...
  if (caller == NoKey || called == NULL)
    return;

  params = GetParams(caller, NULLDefTableKeyList);    

  if (params == NULLDefTableKeyList)
    return;

  if(IsCoercible(
      GetType(k, NoType), 
      GetType(IthElemDefTableKeyList(params, arg_num), NoType), pos, scope, 1))
    return;
