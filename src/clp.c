...
int OutputCIL = 0;
int LongErrorReporting = 0;
DefTableKey MainClassName = NoKey;
static int MainClassName_count = 0;
DefTableKey OutputFile = NoKey;
static int OutputFile_count = 0;
DefTableKeyList LibFiles = NULLDefTableKeyList;
DefTableKey SourceFile = NoKey;
static int error_count = 0;

#define NUMOPTS 6

static opt opts[] = {
    { "-il", BOOL , &OutputCIL, NULL, NULL },
    { "-l", BOOL , &LongErrorReporting, NULL, NULL },
    { "-m", STRVAL | SPACED, &MainClassName_count, &MainClassName, NULL },
    { "-o", STRVAL | SPACED, &OutputFile_count, &OutputFile, NULL },
    { "-I", STRVAL | PLURAL | JOINED, NULL, NULL, &LibFiles },
...
#endif
{
  fprintf (stderr, "Sather-K Compiler Halle, version 0.3.5-347\n");
  fprintf (stderr, "Usage: %s [options] parameters\n", prog);
  fprintf (stderr, "  Items marked with * may be repeated.\n");
  fprintf (stderr, "  Options:\n");
  fprintf (stderr, "    -il      output intermediate language\n");
  fprintf (stderr, "    -l       long error reporting\n");
  fprintf (stderr, "    -m string Name of the main class, default is input filename in upper case\n");
  fprintf (stderr, "    -o string output to file, default is lower case name of mainclass with appropriate ending\n");
  fprintf (stderr, "    -Istring adds another file to the input stream. This option can be used more than once*\n");
...
#endif
{
  ...
  if (i < argc) {
    SourceFile = NewKey ();
    SetClpValue (SourceFile, clp_string (argv[i++]), 0);
    CLP_InputFile = SourceFile;
  }
